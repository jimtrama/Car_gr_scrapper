...
#-------------------------------------------------------------------------
url='https://www.car.gr/classifieds/cars/?fs=1&condition=%CE%9A%CE%B1%CE%B9%CE%BD%CE%BF%CF%8D%CF%81%CE%B9%CE%BF&condition=%CE%9C%CE%B5%CF%84%CE%B1%CF%87%CE%B5%CE%B9%CF%81%CE%B9%CF%83%CE%BC%CE%AD%CE%BD%CE%BF&offer_type=sale&pg='
...
pages=int(input("At what page should it end ? :"))
...
for i in range(start_p,pages):
    ...
    try:  
        ...
        print("Error stopped at :",i)
        file=open("pages.txt","w")
        file.write(str(i))
        file.close()

        file=open("lines.txt","w")
        file.write(str(len(money_a)))
        file.close()
